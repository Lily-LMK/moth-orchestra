> Latest clarification: preserve the lower creek flourish. Prototype subtle observer timbre next; audit timestamp precision before treating five-second pulses as evidence. Rings currently share one taxonomic index across observers. See SONIFICATION-DESIGN.md.

> Design reset, 13 September: start with SONIFICATION-DESIGN.md, linked from the README. Lily clarified A plain/B outlined markers, rings per group at the selected rank, and a desired observer sound signature. Marker convention is restored; no observer sound signature has yet been implemented. Next work should follow the small steps in that reference before the older gap-feature plan.

> Latest update, 13 September: CSV/rank/outline repairs are in the working tree. Read the README repair note first. Current suite: 80 passing, eight TODOs. Prioritise listening verification of imports (3 September 2026 and 17 February 2026) before the earlier proposed gap feature. Browser check remains incomplete.
