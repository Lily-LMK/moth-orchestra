...
function loadApp(htmlPath = path.join(__dirname, '..', 'index.html'), overrides = {}) {
  ...
  vm.runInContext(script.slice(0, boundary) + '\nObject.assign(globalThis, {state, DEMO_CSV, UNKNOWN_TAXON, DUET_ECHO_WINDOW_MIN, DUET_CROSS_WINDOW_MIN, midiToFreq, DUET_GESTURES, photoUrlAt, photoSizeForPx, photoCache, photoProgress, photoStatusText, preloadPhotos, cancelPhotoPreload, nightPhotoUrls, photoIsReady, PHOTO_CACHE_MAX, GROUND, GROUND_CEILING_HZ, OFFERABLE_NIGHT_MIN_RECORDS, SHARED_DATE_MIN_RECORDS, VOICE_REGISTERS, voicedFreq, loadedSummary, loadedSentence, offerableNightKeys, MOTH_AXES, MOTH_CLASS_VOICE, MOTH_CLASS_VOICES, mothVoicing, mothCode, mothDecode, mothVoiceLabel, foldIntoRegister, MOTH_GROUND, mothGroundEvents, mothGroundVoicing, mothGroundPitchClasses, scheduleMothGround, MOTH_SPECTRA, MOTH_DECAY, MOTH_BODY_PARTIALS, MOTH_FILTER_OPEN, MOTH_FILTER_CLOSE, MOTH_TILT, MOTH_ATTACK, MOTH_STRETCH, scheduleMothVoice, VOICE_MODE_LABELS, EMERGENCE, emergenceNightShape, emergenceBreathSec, emergenceMemory, emergenceBand, emergenceTones, emergencePianoNotes, emergenceGrains, buildEmergence});', context, { filename: htmlPath });
  return context;
}
