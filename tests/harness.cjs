...
function loadApp(htmlPath = path.join(__dirname, '..', 'index.html'), overrides = {}) {
  ...
  vm.runInContext(script.slice(0, boundary) + '\nObject.assign(globalThis, {state, DEMO_CSV, UNKNOWN_TAXON, DUET_ECHO_WINDOW_MIN, DUET_GESTURES, GROUND, GROUND_CEILING_HZ, OFFERABLE_NIGHT_MIN_RECORDS, SHARED_DATE_MIN_RECORDS});', context, { filename: htmlPath });
  return context;
}
